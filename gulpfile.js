...
const html = () => src('src/index.html').pipe(formatHtml()).pipe(dest('public'))
const css = () => src('src/index.scss').pipe(sass()).pipe(autofixer()).pipe(dest('public/css'))
const img = () => src('src/img/**/*').pipe(imagein()).pipe(dest('public/img'))
const copy = () => src(['src/fonts/**', 'src/videos/**'], { base: 'src' }).pipe(dest('public'))
...
const js = () =>
  rollup({
    ...
    .then(b => b.write({ file: 'public/js/index.js', format: 'iife', sourcemap: IS_RPOD }))
